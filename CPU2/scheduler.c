...
void* sjf_scheduling(void* arg) {
    ...
    while (processes_completed < NUM_PROCESS) {
        ...
        if (shortest_job_index <0 || shortest_job_index>=NUM_PROCESS) {
            current_time++;
        }
        else{
            // 선택된 프로세스를 실행
    ...
    while (processes_completed < NUM_PROCESS) {
        for (int i = 0; i < NUM_PROCESS; i++) {
            if (process_list[i]->cpu_burst > 0) {
                ...
                printf("Running Process &d", &process_list[i]->pid);
...
void* preemptive_sjf_scheduling(void* arg) {
    initialize_gantt_chart(MAX_COMPLETION_TIME);  // 최대 완료 시간 +1로 초기화
...
void* preemptive_priority_scheduling(void* arg) {
    initialize_gantt_chart(MAX_COMPLETION_TIME);
...
void* io_scheduling(void* arg) {
    ...
    while (io_thread_continue) {
        ...
        if (current != NULL) {
            int random_time = rand() % (current->cpu_burst + 1);

            if (current_time >= current->arrival_time + random_time) {
